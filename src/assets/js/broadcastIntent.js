	...
	if (arguments.length > 2) {
		...
		if (phoneNumber) {
			if (isCorrectType('String', phoneNumber)) {
				...
				console.log(`the phone number should be provided as a string. It was '
        + 'provided as ${Object.prototype.toString.call(phoneNumber).slice(8, -1)},' +
		...
		if (body) {
			if (isCorrectType('String', body)) {
				...
				url += Platform.OS === 'ios' ? '&body' +
        `=${encodeURIComponent(body)}` : `?body=${encodeURIComponent(body)}`;
			} else {
				console.log('the body should be provided as a string. It was provided as' +
          `${Object.prototype.toString.call(body).slice(8, -1)},ignoring the value provided`);
			}
		}
